...
public protocol FTPushSDKDelegate: AnyObject {
    /// 注册推送服务成功回调
    /// - Parameters:
...
public class FTPushSDK: NSObject, XGPushDelegate {
    /// 业务介绍
    public private(set) var des = "FTPushSDK是聚合三方腾讯云旗下的TPNS业务功能的一个SDK。"
    public private(set) var version = "3.2.5"
    static let pushConfigSaveKey = "FTPushSDKConfig"
    /// 单例管理
    public static let shared = FTPushSDK()
    /// 需要执行的协议
    public weak var delegate: FTPushSDKDelegate?
    /// 群组Identifier
    ...
    static func savePushConfig(domain: String, accessId:UInt32, accessKey: String) {
        ...
        if let sharedUserDefaults = UserDefaults(suiteName: FTPushSDK.shared.appGroupIdentifier) {
